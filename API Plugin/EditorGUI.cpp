		...
		if (WindowList[editor_stageEditor])
		{
			...
			if (editor_spawnSelected != -1)
			{
				...
				if (eObj->anmclump != nullptr)
				{
					...
					ImGui::InputInt("Tab", &editor_spawnAnmTab);
					if (editor_spawnAnmTab < 0) editor_spawnAnmTab = 0;
					if (editor_spawnAnmTab > 2) editor_spawnAnmTab = 2;

					ImGui::Dummy(ImVec2(0, 10));

					if (editor_spawnAnmTab == 0)
					{
						std::string ptrRead = "ccStageAnm: " + Utilities::HexToString((__int64)(eObj->anmclump));
						ImGui::Text(ptrRead.c_str());

						ImGui::InputFloat("m_float00", &(eObj->anmclump->m_float00));
						ImGui::InputInt("m_int10", &(eObj->anmclump->m_int01_10));
						ImGui::InputInt("m_int80", &(eObj->anmclump->m_int02_80));
						ImGui::InputInt("m_int88", &(eObj->anmclump->m_int03_88));
						ImGui::InputInt("m_int8C", &(eObj->anmclump->m_int04_8C));
						ImGui::InputInt("m_int90", &(eObj->anmclump->m_int05_90));
						ImGui::InputInt("m_int94", &(eObj->anmclump->m_int06_94));
						ImGui::InputInt("m_int98", &(eObj->anmclump->m_int07_98));

						ImGui::Text((const char*)(eObj->anmclump->m_str_40));

						ptrRead = "m_unkptr_E8: " + Utilities::HexToString((__int64)eObj->anmclump->m_unkptr_E8);
						ImGui::Text(ptrRead.c_str());

						ImGui::InputFloat3("m_vector00", &(eObj->anmclump->m_vector00.x));
					}
					else if (editor_spawnAnmTab == 1)
					{
						std::string ptrRead = "ccAdvBaseAnm: " + Utilities::HexToString((__int64)(eObj->anmclump->m_advBaseAnm));
						ImGui::Text(ptrRead.c_str());

						ccAdvBaseAnm * baseAnm = eObj->anmclump->m_advBaseAnm;

						ImGui::InputFloat("Animation Speed", &(baseAnm->m_anmSpeed));
						ImGui::Checkbox("Display?", (bool*)&(baseAnm->m_display));
						ImGui::InputInt("m_flagA0", &(baseAnm->m_flag_A0));
						ImGui::InputInt("m_flagA8", &(baseAnm->m_flag_A8));
						ImGui::InputInt("m_isUpdated", &(baseAnm->m_isUpdated));
						ImGui::InputFloat3("m_position", &(baseAnm->m_position.x));
						ImGui::InputFloat3("m_scale", &(baseAnm->m_scale.x));

						ptrRead = "m_qword10: " + Utilities::HexToString(baseAnm->m_qword_10);
						ptrRead = "m_qword18: " + Utilities::HexToString(baseAnm->m_qword_18);
						ptrRead = "m_qword20: " + Utilities::HexToString(baseAnm->m_qword_20);
					}
					else if (editor_spawnAnmTab == 2)
					{
						std::string ptrRead = "nuccAnm: " + Utilities::HexToString((__int64)(eObj->anmclump->m_advBaseAnm->m_nuccAnm));
						ImGui::Text(ptrRead.c_str());
